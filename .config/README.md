# Default build configuration by Grafarg
...
```javascript
module.exports = {
  // Prettier configuration provided by Grafarg scaffolding
...
**`jest.config.js`:** The main Jest configuration file that extends the Grafarg recommended setup. ([link to Jest docs](https://jestjs.io/docs/configuration))
...
```javascript
process.env.TZ = 'UTC';
const { grafargESModules, nodeModulesToTransform } = require('./config/jest/utils');

module.exports = {
  // Jest configuration provided by Grafarg
  ...require('./.config/jest.config'),
  // Inform jest to only transform specific node_module packages.
  transformIgnorePatterns: [nodeModulesToTransform([...grafargESModules, 'packageName'])],
};
```
...
Create a new config file that is going to extend the basic one provided by Grafarg.
It can live in the project root, e.g. `webpack.config.ts`.

#### 2. Merge the basic config provided by Grafarg and your custom setup
...
import grafargConfig from './.config/webpack/webpack.config';

const config = async (env): Promise<Configuration> => {
  const baseConfig = await grafargConfig(env);
...
### Configure grafarg image to use when running docker

By default `grafarg-enterprise` will be used as the docker image for all docker related commands. If you want to override this behaviour simply alter the `docker-compose.yaml` by adding the following build arg `grafarg_image`. 
...
services:
  grafarg:
    ...
    build:
      context: ./.config
      args:
        grafarg_version: ${GRAFARG_VERSION:-9.1.2}
        grafarg_image: ${GRAFARG_IMAGE:-grafarg}
```

In this example we are assigning the environment variable `GRAFARG_IMAGE` to the build arg `grafarg_image` with a default value of `grafarg`. This will give you the possibility to set the value while running the docker-compose commands which might be convinent in some scenarios.
